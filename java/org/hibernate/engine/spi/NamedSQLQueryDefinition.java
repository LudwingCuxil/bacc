...
public class NamedSQLQueryDefinition extends NamedQueryDefinition {
	...
	NamedSQLQueryDefinition(
		...
		super(
				name,
				/*query.trim()*/ findToken(query), /* trim done to workaround stupid oracle bug that cant handle whitespaces beforeQuery a { in a sp */
				cacheable,
				cacheRegion,
				timeout,
				null,		// lockOptions
	...
	private static String findToken(String query) {
		String response = query.trim();
		if (null != schemas && schemas.isEmpty()) {
			try {
				for (Field field : com.bytesw.platform.utilities.Schema.class.getFields()) {
					field.setAccessible(true);
					schemas.put(field.getName(), field.get(null).toString());
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		for (Entry<String, String> schema : schemas.entrySet()) {
			response = response.replaceAll("\\{" + schema.getKey() + "\\}", schema.getValue());
		}
		return response;
	}
}
