import {
  ...
  Output, SimpleChange, AfterViewChecked
} from '@angular/core';
...
@Component({
  selector: 'pl-dual-list',
  template: `
    <div>
      <form>
        <div class="col-lg-12">
          <div class="content-duallist col-lg-5">
            <div class="record-picker" [ngClass]="editMode? 'disabled' : ''">
              <ul [ngStyle]="{'max-height': height, 'min-height': height}" [ngClass]="{over:available.dragOver}"
                  (drop)="drop($event, confirmed)" (dragover)="allowDrop($event, available)" (dragleave)="dragLeave()">
                <li *ngFor="let item of available.list; let idx=index;"
                    (click)="selectItem(available.pick, item, false); shiftClick($event, idx, available, item)"
            ...
            <div style="margin-top:8px;">
              <button type="button" class="btn btn-success" (click)="selectAll(available)"
                      [disabled]="isAllSelected(available)"><i class="material-icons md-18">done_all</i> Seleccionar Todo
                <div class="ripple-container"></div>
              </button>
              <button type="button" class="btn btn-outline-danger" (click)="selectNone(available)"
                      [disabled]="!isAnySelected(available)"><i class="material-icons md-18">clear</i> Deseleccionar
                <div class="ripple-container"></div>
              </button>
            </div>
          </div>
          <div class="col-lg-2 text-center btn-group-block">
            <button type="button" name="addBtn" class="btn btn-primary"
                    (click)="moveItem(available, confirmed, null, true)"
                    [disabled]="disableButton()"><i class="material-icons">chevron_right</i>
            </button>
            <button type="button" name="removeBtn" class="btn btn-primary"
                    (click)="moveItem(confirmed, available, null,true)"
                    [disabled]="confirmedButton()"><i class="material-icons">chevron_left</i>
            </button>
          </div>
          <div class="content-duallist col-lg-5">
            <div class="record-picker col-lg-10 col-sm-10" [ngClass]="editMode? 'disabled' : ''">
              <ul [ngStyle]="{'max-height': height, 'min-height': height}" [ngClass]="{over:confirmed.dragOver}"
                  (drop)="drop($event, available)" (dragover)="allowDrop($event, confirmed)" (dragleave)="dragLeave()">
                <li *ngFor="let item of confirmed.list; let idx=index;"
                    (click)="selectItem(confirmed.pick, item, true); shiftClick($event, idx, confirmed, item)"
            ...
            <div class="col-lg-2 col-sm-2 text-center btn-group-block">
              <button type="button" class="btn" (click)="selectUp()"
                      [disabled]="disableUp"><i class="material-icons md-18">keyboard_arrow_up</i>
                <div class="ripple-container"></div>
              </button>
              <button type="button" class="btn" (click)="selectDown()"
                      [disabled]="disableDown"><i class="material-icons md-18">keyboard_arrow_down</i>
                <div class="ripple-container"></div>
              </button>
            </div>
            <div style="margin-top:8px;" class="row col-lg-12">
              <button type="button" class="btn btn-success" (click)="selectAll(confirmed)"
                      [disabled]="isAllSelected(confirmed)"><i class="material-icons md-18">done_all</i> Seleccionar Todo
                <div class="ripple-container"></div>
              </button>
              <button type="button" class="btn btn-outline-danger" (click)="selectNone(confirmed)"
                      [disabled]="!isAnySelected(confirmed)"><i class="material-icons md-18">clear</i> Deseleccionar
                <div class="ripple-container"></div>
              </button>
            </div>
          </div>
  ...
  `,
  styleUrls: ['./record-picker.css']
})

export class DualListAccountComponent implements DoCheck, OnChanges, AfterViewChecked {
  ...
  @Input() destinationAssing: Array<any> = [];
  @Input() editMode = false;
  @Output() destinationChange = new EventEmitter();
  @Output() onAdded = new EventEmitter();
  @Output() onRemoved = new EventEmitter();
  @Output() selectAssign = new EventEmitter();


  available: BasicList;
  confirmed: BasicList;
  selectTemp: BasicList = new BasicList(DualListAccountComponent.CONFIRMED_LIST_NAME);
  availableCopy: BasicList = new BasicList(DualListAccountComponent.CONFIRMED_LIST_NAME);
  confirmedCopy: BasicList = new BasicList(DualListAccountComponent.CONFIRMED_LIST_NAME);

  private sourceDiffer: any;
  private destinationDiffer: any;
  private disableUp = true;
  private disableDown = true;
  ...
  ngOnChanges(changeRecord: { [key: string]: SimpleChange }) {
    ...
    if (changeRecord['source']) {
      this.available = new BasicList(DualListAccountComponent.AVAILABLE_LIST_NAME);
      this.updatedSource();
      this.updatedDestination();
    }

    if (changeRecord['destination']) {
      this.confirmed = new BasicList(DualListAccountComponent.CONFIRMED_LIST_NAME);
      this.updatedDestination();
      this.updatedSource();
    }
  }
  ...
  drop(event: DragEvent, list: BasicList) {
    if (!this.editMode) {
        ...
        if (list === this.available) {
          this.moveItem(this.available, this.confirmed, null, true);
        } else {
          this.moveItem(this.confirmed, this.available, null, true);
        }
    }
  }

  trueUp(external: boolean) {
    let changed = false;
    ...
    while ((pos -= 1) >= 0) {
      ...
      if (mv.length === 0) {
        // Not found so remove.
        let removed = this.destination.splice(pos, 1);
        if (external) {
          this.onRemoved.emit(removed[0]);
        }
        changed = true;
      }
    }
  ...
  moveItem(source: BasicList, target: BasicList, item: any = null, external: boolean = false) {
    let i = 0;
...
//					if ( target.list.filter( trg => { return trg[this.key] === mv[0][this.key]; }).length === 0) {
    ...
    // Update destination
    this.trueUp(external);
  }

  disableButton() : boolean {
    if (!this.editMode && this.available.pick.length > 0)
      return false;
    return true;
  }

  confirmedButton() : boolean {
    if (!this.editMode && this.confirmed.pick.length > 0)
      return false;
    return true;
  }
  ...
  selectItem(list: Array<any>, item: any, assigned?: boolean) {
    ...
    if (pk.length > 0) {
      ...
      const findTemp = this.selectTemp.list.filter((e: any) => {
        return Object.is(e, item);
      })[0];
      if (findTemp) {
        const iex = this.selectTemp.list.indexOf(findTemp);
        if (iex !== -1)
         this.selectTemp.list.splice(iex, 1);
      }
    } else {
      list.push(item);
      if (!this.editMode && assigned){
        const valid = this.selectTemp.list.filter((e:any) => {return Object.is(e, item)})[0];
        if (!valid) {
          const position = this.confirmed.list.filter((e:any) => {return Object.is(e, item)})[0];
          const idxc = this.confirmed.list.indexOf(position);
          let posSplice = undefined;
          let posPush = undefined;
          for (let i = 0, len = this.selectTemp.list.length; i < len; i += 1) {
            const idxs = this.confirmed.list.indexOf(this.selectTemp.list[i]);
            if (idxs > idxc) {
                posSplice = i;
                posPush = undefined;
//              const duplicate = this.selectTemp.list.filter((dupl:any) => {return Object.is(dupl, item)})[0];
//              if (!duplicate)
//                this.selectTemp.list.splice(i, 0, item);
            } else {
//              this.selectTemp.list.push(item);
              posPush = i;
              posSplice = undefined;
            }
          }
          if (this.selectTemp.list.length === 0) {
            this.selectTemp.list.push(item);
          } else {
            if (posSplice === 0 || posSplice)
              this.selectTemp.list.splice(posSplice, 0, item);
            if (posPush === 0 || posPush)
              this.selectTemp.list.push(item);
          }
        }
      }
    }
    if (assigned && !this.editMode) {
        this.disableUp = false;
        this.disableDown = false;
        for(let i = 0, len = this.selectTemp.list.length; i < len; i += 1) {
          const findAssigned = this.confirmed.list.filter((e:any) => {return Object.is(e, this.selectTemp.list[i])})[0];
          if (findAssigned) {
            const index = this.confirmed.list.indexOf(findAssigned);
            if (this.confirmed.list.length === index + 1)
  ...
  selectUp() {
    if (this.selectTemp.list.length > 0){
      this.destinationAssing = [];
      for(let i = 0, len = this.selectTemp.list.length; i < len; i += 1) {
        const findAssigned = this.confirmed.list.filter((e:any) => {return Object.is(e, this.selectTemp.list[i])})[0];
        if (findAssigned) {
          const index = this.confirmed.list.indexOf(findAssigned);
          this.confirmed.list.splice(index, 1);
          this.confirmed.list.splice(index - 1, 0, findAssigned);
          if ((index - 1) === 0)
            this.disableUp = true;
          if (index < this.confirmed.list.length)
            this.disableDown = false;
        }
      }
      for (let i = 0, len = this.confirmed.list.length; i < len; i += 1) {
        ...
        if (mv)
          this.destinationAssing.push(mv);
      }
      this.selectAssign.emit(this.destinationAssing);
    }
  }
    ...
    if (this.selectTemp.list.length > 0) {
      ...
      for (let i = 0, len = this.selectTemp.list.length; i < len; i += 1) {
        ...
        if (findAssigned) {
          const index = this.confirmed.list.indexOf(findAssigned);
          this.confirmed.list.splice(index, 1);
          this.confirmed.list.splice(index + this.selectTemp.list.length, 0, findAssigned);
          let indexTemp = this.selectTemp.list.indexOf(this.selectTemp.list[i]);
          if ((index + indexTemp + 2) === this.confirmed.list.length)
            this.disableDown = true;
          if ((index + 1) > 0)
            this.disableUp = false;
        }
  ...
  selectNone(source: BasicList) {
    source.pick.length = 0;
    this.selectTemp.list = [];
    this.disableUp = true;
    this.disableDown = true;
  }

  isAllSelected(source: BasicList) {
    if (source.list.length === 0 || source.list.length === source.pick.length || this.editMode) {
      return true;
    }
    return false;
  }
  ...
  listCopy() {
    if (this.confirmedCopy.list.length === 0) {
      this.confirmedCopy = JSON.parse(JSON.stringify(this.confirmed));
      this.availableCopy = JSON.parse(JSON.stringify(this.available));
    }
  }
