...
import { ClientComponent as ParentClientComponent } from 'backoffice-ace/src/app/core/client-core/client.component'
...
@Component({
  selector: 'pl-client',
  template : require('backoffice-ace/src/app/core/client-core/client.component.html'),
  providers : [TypedocService,ClientService]
})
...
export class ClientComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.typedocService.getDocuments({number: 0, size: 1500})
      .then((typedocs:any) => {
  ...
  search() : void {
    ...
    this.busy.then((clients)=> {this.clients = clients.content;
        for (var i = 0; i < this.clients.length; i++) {
          ...
          let result = this.typedocs.filter(item => item.codigo.indexOf(this.clients[i].tipoDeIdentificacion) !== -1);
          if(result.length>0)
